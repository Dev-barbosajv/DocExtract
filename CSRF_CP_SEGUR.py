...
diretorio = 'PDF_PATRONAL'

# Lista para armazenar os dados de todos os arquivos PDF
todos_os_dados = []

# Regex fornecido
pattern = r'(\d{4}-\d{2}\s-\s(?:CSRF|CP-SEGUR\.|CP-PATRONAL))\s+(\d{2}/\d{4})\s+(\d{2}/\d{2}/\d{4})\s+(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)\s+(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)\s+(DEVEDOR)'
...
nome_arquivo_csv = 'receitas_csrf.csv'
